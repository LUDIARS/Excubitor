  ...
  it('issues a credential per launch and strips the issuer secret from the child env', async () => {
    const fetchImpl = vi.fn(async (_url: string | URL | Request, init?: RequestInit) => {
      ...
      expect(body).toMatchObject({
        ...
        target_client_secret: 'ex-generated-EducationLab-secret-0123456789abcdef',
      });
      return new Response(JSON.stringify({
        targetProjectKey: 'EducationLab',
        launchId: body.launch_id,
        clientId: 'EducationLab-client-id',
    ...
    const env = await prepareSpawnEnv(service(), {
      ...
      targetClientSecret: 'ex-generated-EducationLab-secret-0123456789abcdef',
    });

    expect(env).toMatchObject({
      CERNERE_PROJECT_CLIENT_ID: 'EducationLab-client-id',
      CERNERE_PROJECT_CLIENT_SECRET: 'ex-generated-EducationLab-secret-0123456789abcdef',
