...
import type {
  ...
  CommitInfo, RecentLogLine, ServiceEnvConfig, ServiceFunctionMetrics,
  FunctionMetricAggregate, FunctionMetricSort,
} from '../lib/api';
import {
  ...
  fetchServiceEnvConfig, saveServiceEnvConfig, fetchFunctionMetrics,
} from '../lib/api';
...
export default function Monitor() {
  ...
  const [metricCode, setMetricCode] = useState('concordia');
  ...
  const metricCandidates = useMemo(() => {
    ...
    for (const c of rows.flatMap((p) => p.components)) {
      if (c.disabled || managedPorts(c).length === 0 || byCode.has(c.code)) continue;
      byCode.set(c.code, c);
    }
  ...
  useEffect(() => {
    if (metricCandidates.length === 0) return;
    if (metricCandidates.some((c) => c.code === metricCode)) return;
    setMetricCode(metricCandidates.find((c) => c.code === 'concordia')?.code ?? metricCandidates[0]!.code);
  }, [metricCandidates, metricCode]);
    ...
    <div className="monitor">
      ...
      {metricCandidates.length > 0 && (
        <FunctionMetricsPanel
          ...
          selectedCode={metricCode}
          onSelectCode={setMetricCode}
        />
      )}
    ...
    <section className="function-metrics-panel">
      <div className="function-metrics-head">
        ...
        <div className="function-metrics-controls">
          ...
          <button disabled={busy} onClick={() => void fetchFunctionMetrics(selectedCode, { limit: 30, kind: kind || undefined, sort }).then((r) => {
            setData(r);
            setError(null);
