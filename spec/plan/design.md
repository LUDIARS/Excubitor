...
### 15.1 ウィンドウ無し起動 (req1)
- `process/manager.ts` の spawn に `windowsHide: true` を追加。 従来 `detached: true` のみで Windows ではコンソール窓が出ていた。 detached は維持 (Excubitor 再起動でサービスを道連れにしない)。 再起動は既存 control restart。
