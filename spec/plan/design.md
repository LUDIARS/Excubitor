...
Corpus が大規模 Hub の役割を担っていくにあたり、サービスの lifecycle と観測を一貫した
ローカル契約で扱う必要がある。ただし HTTP server の停止を lifecycle 停止へ波及させないため、
**起動 / 再起動は local supervisor、ログ集約 / エラー検知 / 死活モニターは Ex backend**
という process boundary を設ける。
...
| **Excubitor backend** (本書) | **サービス監視・設定・ログ UI** — catalog / 死活監視 / ログ集約 / 設定編集 / エラー検知 / 自動修正。lifecycle API は local supervisor への proxy。 |
| **Excubitor local supervisor** | **lifecycle authority** — Ex 自身と catalog service の start / stop / restart / status、restart policy、PID/process handle、operation serialization。 |
...
```
operator ── excubitorctl ──┐
                           ├── versioned local IPC ── persistent supervisor
Web UI ── control API ─────┘                              │
                                                        ├── Ex backend :17332
                                                        └── catalog services

Corpus ── HTTP ── Ex backend ── scanner / log / error / config / SQLite
                       └────── control routes are IPC proxies only
```

supervisor は Ex backend と別プロセスであり、backend の HTTP listener や event loop が停止しても
CLI から利用できる。CLI と API が process manager を別々に呼ぶ二重 writer は禁止する。
...
- 起動: supervisor=`npm run service`、client=`npm run ctl -- ...` / `excubitorctl ...`
...
local supervisor 起動
  ...
  → Ex backend を起動
      └ scanner / log bus / error detector 開始
```

- `dev-services.json` の選択 (goal-set 等) は、catalog の `autostart` フラグ + UI トグルに移行。
- Web UI の操作は control API → local IPC の順で supervisor に代理実行する。
- HTTP 停止時は `npm run ctl -- service <code> <action> --json` を直接実行する。
...
| POST | `/api/hub/services/:code/control` | local supervisor への start/stop/restart proxy (Corpus からの操作、要認可) |
...
| POST | `/api/v1/services/:code/control` | `{action: start\|stop\|restart}` を local supervisor IPC へ proxy |
...
HTTP response は supervisor の operation ID / state / error を保持する。backend 内で直接
spawn/kill へ fallback してはならない。
...
## 16. v0.7 — HTTP 非依存の local-control plane (2026-07-12)

### 16.1 採用判断

one-shot CLI だけでは command 終了後に exit listener / restart policy / operation queue が消える。
また Ex backend 自身からの restart は、kill 後の spawn を同じプロセスで完了できない。
このため、OS が起動・復旧する **persistent local supervisor** と、そこへ IPC request を送る
stateless **`excubitorctl` client** を採用する。

### 16.2 所有権と禁止事項

- supervisor だけが Ex backend / catalog service の process lifecycle を変更する。
- backend は monitor / config editor / log viewer / error triage を主責務とする。
- Web control API は IPC proxy。IPC 不通時に in-process spawn/kill へ fallback しない。
- CLI/API の request は target ごとに直列化し、operation ID で再送を冪等化する。
- state / endpoint は repo の build output ではなく runtime data として永続化する。

### 16.3 CLI 契約

```text
npm run ctl -- service <code> <start|stop|restart|status> --json
npm run ctl -- excubitor <start|stop|restart|status> --json
npm run ctl -- service <code> <kill-port|claude-port-fix> [--port=<n>] --json

# package bin を導入した環境
excubitorctl service <code> <action> --json
excubitorctl excubitor <action> --json
```

JSON mode の stdout は単一の protocol response とし、診断は stderr に分離する。非 JSON mode と
exit code は人手運用向け、API proxy は JSON schema と `ok` を検証する。

### 16.4 自己 restart handoff

supervisor は Ex restart/stop を受理・永続化して operation ID を返した後、deferred operation として
backend を停止する。これにより HTTP response の送信元が終了しても supervisor が新 backend の
spawn と readiness 判定を完了できる。PID 単独ではなく process identity を照合し、別プロセスを
誤停止しない。
...
- 起動直後の identity 照合を最大 **10s** 再試行するため、**即死 → その間に pid が再利用された**場合は
  無関係なプロセスが tolerance (30s) 内の作成時刻を持ちうる。単発照合より窓が広い。win32 の pid
  再利用は即時ではないため実害は低いが、adopt 後の停止は `taskkill /T /F` である点に留意する。
  - 旧値は 3s。照合 1 回のコスト (powershell + `Get-Process`) が**アイドルでも 1.0〜2.5s**
    (2026-08-12 実測) で、起動と同時に走る他プロセスが CPU を取ると 1 回目で使い切って
    リトライが入らず、正常に listen しているプロセスまで「照合不能」で失敗扱いになっていた
    (実測: Genius が応答しているのに supervisor は stopped / pid=null)。照合が返す値は
    正しく、**予算が足りていなかった**。伸ばしても成功時の所要は変わらない (照合できた時点で
    抜ける) が、即死したサービスの失敗検知はその分遅れる。
