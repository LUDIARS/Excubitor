...
## 14. 統合の2軸での位置づけ — バックエンド・インフラ統合 (2026-06-08)

LUDIARS の統合は2軸に確定: **フロント統合 = Corpus / バックエンド・インフラ
統合 = Excubitor**。

- **Excubitor = バックエンド・インフラ統合 (ops / control plane)**。 Corpus / 各
  サービスを **外から監視・制御**する (catalog/scanner/control/process/log/
  auto-fix + 集中設定 + Secret 注入)。 loopback 専有
- **プレイヤー入口にはしない**。 プレイヤーが叩く公開エンドポイント (フロント
  統合 = エッジ + UI 集約 + ambient SSO host) は **Corpus app** が担う。 ops と
  公開 ingress を混ぜると役割が壊れるため、 Excubitor は player 経路の外に置く
- プレイヤーは control plane (Excubitor) に触れない、 が原則

3 平面の整理:

| 平面 | 担う |
|---|---|
| レンダラ (ライブラリ) | `corpus-renderer` |
| フロント統合 (プレイヤー入口) | Corpus app |
| **バックエンド・インフラ統合 (ops/control plane)** | **Excubitor (本書)** |

詳細: `Cernere/spec/feature/corpus-frontend-ui.md` §13 / `Corpus/DESIGN.md` §15。
