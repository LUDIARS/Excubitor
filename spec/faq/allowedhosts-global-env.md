---
tags: [vite, allowedHosts, env, global-env, PrivateProject]
date: 2026-06-30
---

# Q: Vite dev server で `*.PrivateProject.dev` がブロックされる

## 症状

```
Blocked request. This host ('ex.PrivateProject.dev') is not allowed.
To allow this host, add 'ex.PrivateProject.dev' to server.allowedHosts in vite.config.js.
```

Cloudflare Tunnel 経由 (`*.PrivateProject.dev`) でアクセスすると Vite が拒否する。
...
1. `catalog/services.yaml` の `global.env` セクション (ファイル先頭) に定義:
   ...
   global:
     env:
       LUDIARS_ALLOWED_HOSTS: ".PrivateProject.dev"
   ```
...
4. Vite の先頭ドット規約: `.PrivateProject.dev` は `*.PrivateProject.dev` 全サブドメインを許可する。
